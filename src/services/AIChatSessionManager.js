...
const AIChatExportService = require('./AIChatExportService');
...
class AIChatSessionManager {
  constructor(client) {
    ...
    this._timeouts = [];
    this.exportService = new AIChatExportService((channel) => this._findChannelOwner(channel), client);
  }
...
**ملاحظة:** يتم حذف القنوات غير النشطة بعد ${config.aiChat.inactivityTimeoutHours} ساعة.`
  ...
  async _handleExport(interaction, channel, format) {
    return this.exportService.handleExport(interaction, channel, format);
  }
